...
def make_vpc(**kwargs):
    ...
    try:
        ...
        if verify_vpc(kwargs['Vpc_name'], kwargs['Region']) == 1:
            ...
            logging.info(f'Create VPC: {kwargs["Vpc_name"]} in {kwargs["Region"]}...' )
            print(f'Create VPC: {kwargs["Vpc_name"]} in {kwargs["Region"]}...' )
    except Exception as err:
        logging.info(f'Create VPC {kwargs["Vpc_name"]} error {err} in {kwargs["Region"]} logged in vpc.log...')
        print(f'Create VPC {kwargs["Vpc_name"]} error {err} in {kwargs["Region"]} logged in vpc.log...')
...
def update_vpc(**kwargs):
    ...
    try:
        ...
        logging.info(f'Additional CIDR: {kwargs["Cidr"]} for VPC {kwargs["Vpc_Id"]}in {kwargs["Region"]}...')
        print(f'Additional CIDR: {kwargs["Cidr"]} for VPC {kwargs["Vpc_Id"]}in {kwargs["Region"]}...')
    except Exception as err:
        logging.info(f'Additonal CIDR {kwargs["Cidr"]} error {err} in {kwargs["Region"]} logged in vpc.log...')
        print(f'Additonal CIDR {kwargs["Cidr"]} error {err} in {kwargs["Region"]} logged in vpc.log...')
...
def get_VpcId(Vpc_name, region='us-east-1'):
    ...
    try:
        ...
        logging.info(err)
        logging.info(f'Logging get_VpcId {Vpc_name} error {err} in {region} to vpc.log...')
        print(f'Logging get_VpcId {Vpc_name} error {err} in {region} to vpc.log...')
...
def destroy_vpc(Vpc_id, region='us-east-1'):
    ...
    try:
        ...
        logging.info(f'Logging destroy_vpc {Vpc_id} error {err} in {region} to vpc.log...')
        print(f'Logging destroy_vpc {Vpc_id} error {err} in {region} to vpc.log...')
...
def erase_vpcs(region='us-east-1'):
    ...
    try:
        ...
        logging.info('Logging erase_vpcs error {err} in {region} to vpc.log...')
        print('Logging erase_vpcs error {err} in {region} to vpc.log...')
